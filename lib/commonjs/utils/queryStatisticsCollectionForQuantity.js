...
async function queryStatisticsCollectionForQuantity(identifier, options, from, to, unit) {
  ...
  const rawResponse = await _nativeTypes.default.queryStatisticsCollectionForQuantity(identifier, actualUnit, from.toISOString(), toDate.toISOString(), options, (error, data) => console.log(JSON.stringify(data)));
